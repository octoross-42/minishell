...
{
	if (minishell->previous_fork == PIPE)
...
{
	if ((minishell->previous_fork == AND) || (minishell->previous_fork == OR))
	{
		if (minishell->last_cmd == -1)
		{
			if (minishell->previous_fork == AND)
				ft_exec_ast(ast, minishell, true);
			else if (minishell->previous_fork == OR)
		...
		{
			if (minishell->previous_fork == OR)
				ft_exit_minishell(minishell, WEXITSTATUS(minishell->status));
			else if (minishell->previous_fork == AND)
				ft_exec_ast(ast, minishell, true);
		}
	}
}
...
void	ft_exec_fork(t_ast *ast, t_minishell *minishell)
{
	...
	if (ast->left->cmd || (minishell->previous_fork != PIPE))
		minishell->last_cmd = pid;
	else
		minishell->last_cmd = -1;
	ft_pipe_parent(ast, minishell, pipefd);
	minishell->previous_fork = ast->token;
